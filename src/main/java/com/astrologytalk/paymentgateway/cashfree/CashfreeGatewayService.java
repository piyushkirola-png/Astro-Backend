package com.astrologytalk.paymentgateway.cashfree;
...
public class CashfreeGatewayService {
    ...
    /**
     * Initiate Cashfree order + payment link for a pending Payment.
     * Updates the Payment object with paymentLink.
     *
     * @return true if successful (paymentLink set), false otherwise
     */
    ...
    public boolean initiate(Payment payment, User user) {
        try {
            ...
            orderMeta.put("return_url", frontendUrl + "/user/payments?status=return");
            orderMeta.put("notify_url", frontendUrl.replace(":5170", ":8080") + "/api/webhooks/cashfree");
            ...
            ResponseEntity<Map> orderResp = restTemplate.exchange(
                    baseUrl() + "/orders", HttpMethod.POST, orderReq, Map.class);
            ...
            String expiry = LocalDateTime.now().plusHours(24)
                    .format(DateTimeFormatter.ISO_LOCAL_DATE_TIME) + "Z";
            linkBody.put("link_expiry_time", expiry);

            Map<String, Object> linkMeta = new HashMap<>();
            linkMeta.put("return_url", frontendUrl + "/user/payments?status=return");
            linkMeta.put("notify_url", frontendUrl.replace(":5170", ":8080") + "/api/webhooks/cashfree");
            ...
            ResponseEntity<Map> linkResp = restTemplate.exchange(
                    baseUrl() + "/links", HttpMethod.POST, linkReq, Map.class);
            ...
            log.error("[Cashfree] 4xx on initiate for {}: {} — {}",
                    payment.getGatewayOrderId(), e.getStatusCode(), e.getResponseBodyAsString());
            return false;
        } catch (org.springframework.web.client.HttpServerErrorException e) {
            log.error("[Cashfree] 5xx on initiate for {}: {} — {}",
                    payment.getGatewayOrderId(), e.getStatusCode(), e.getResponseBodyAsString());
            return false;
        } catch (Exception e) {
            log.error("[Cashfree] Unexpected error on initiate for {}: {}",
