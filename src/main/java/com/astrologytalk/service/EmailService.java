...
public class EmailService {
     ...
     * Runs async so webhook response is fast.
     */
    @Async
    public void sendInvoiceEmail(User user, Payment payment) {
        ...
        try {
            ...
            log.info("[Email] Invoice sent to {} for order {}", user.getEmail(), payment.getGatewayOrderId());

        } catch (Exception e) {
            log.error("[Email] Failed to send invoice for order {}: {}",
    ...
    private String buildInvoiceHtml(User user, Payment payment) {
        ...
        if ("REPORT".equalsIgnoreCase(payment.getCategoryCode())) {
            ...
            benefitLine = "<p style=\"margin:8px 0;color:#555;\">Your complete Kundali Report is now unlocked. View it anytime from your dashboard.</p>";
        } else {
            productLine = "AI Chat Wallet Recharge";
            int secs = payment.getSecondsCredited() != null ? payment.getSecondsCredited() : 0;
            int mins = secs / 60;
            benefitLine = "<p style=\"margin:8px 0;color:#555;\">You have added <strong>" + secs
                    + " seconds (" + mins + " minutes)</strong> to your chat wallet.</p>";
        }
        ...
        return "<!DOCTYPE html>"
            ...
            + "<h2 style=\"margin:0 0 6px;color:#17120c;\">Thank you, " + escape(firstName) + " 🙏</h2>"
            ...
            + escape(payment.getGateway()) + "</td></tr>"
            ...
            + "<p style=\"color:#a29a86;font-size:12px;margin:0;\">Need help? Reply to this email. — Team " + appName + "</p>"
            + "</div></body></html>";
    }
    ...
    private String buildWelcomeHtml(User user) {
        ...
        return "<!DOCTYPE html>"
            + "<html><body style=\"font-family:Arial,sans-serif;background:#faf8f4;margin:0;padding:24px;\">"
            + "<div style=\"max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:28px;border:1px solid #e7e3d8;\">"
            + "<h2 style=\"margin:0 0 6px;color:#17120c;\">Welcome, " + escape(firstName) + " 🙏</h2>"
            + "<p style=\"color:#585043;\">We're delighted to have you at <strong>" + appName + "</strong>.</p>"
            ...
            + "<hr style=\"border:none;border-top:1px solid #e7e3d8;margin:20px 0;\">"
            + "<p style=\"color:#a29a86;font-size:12px;margin:0;\">— Team " + appName + "</p>"
            + "</div></body></html>";
    }
