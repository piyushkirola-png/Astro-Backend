...
public class KundaliPdfService {
  ...
  public byte[] generate(User user) {
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      try {
        List<PlanetaryPositionResponse> planets =
            kundaliService.getPlanetaryPositions(user.getId());
      ...
      log.error("[Kundali PDF] Generation failed for user {}: {}", user.getId(), e.getMessage(), e);
  ...
  private void addHeader(Document doc, User user) throws DocumentException {
    ...
    Paragraph sub = new Paragraph(user.getName() != null ? user.getName() : "User", subFont);
  ...
  private void addChartsSection(Document doc, List<KundaliChartResponse> charts)
  ...
  private void addPlanetaryTable(Document doc, List<PlanetaryPositionResponse> planets)
      throws DocumentException {
    ...
    for (PlanetaryPositionResponse p : planets) {
      ...
      addDataCell(
          table,
          p.getHouse() != null ? String.valueOf(p.getHouse()) : "—",
          cellFont,
  ...
  private void addDashaSection(Document doc, List<DashaPeriodResponse> dashas)
      throws DocumentException {
    ...
    for (DashaPeriodResponse d : dashas) {
      ...
      String dates =
          (d.getStartDateFormatted() != null ? d.getStartDateFormatted() : "—")
              + " to "
              + (d.getEndDateFormatted() != null ? d.getEndDateFormatted() : "—");

      Paragraph heading = new Paragraph(d.getPlanet() + " Mahadasha (" + dates + ")", labelFont);
  ...
  private void addFooter(Document doc) throws DocumentException {
    ...
    Paragraph note = new Paragraph("Questions? Contact " + COMPANY_EMAIL, noteFont);
